package com.boop.love_sing.app.shiro.jwt;
...
import com.boop.love_sing.common.util.JsonConvertUtil;
...
//            return false;
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        if (JedisUtil.exists("shiro:refresh_token:" + account)) {
            ...
            if (JwtUtil.getClaim(token, "currentTimeMillis").equals(currentTimeMillisRedis)) {
                ...
                JedisUtil.setObject("shiro:refresh_token:" + account, currentTimeMillis, 180000);
    ...
    private void response401(ServletResponse response, String msg) {
        ...
        try (PrintWriter out = httpServletResponse.getWriter()) {
            String data = JsonConvertUtil.objectToJson(Result.fail("无权访问"+msg));
            out.append(data);
        } catch (IOException e) {
            logger.error("直接返回Response信息出现IOException异常:{}", e.getMessage());
            throw new ForestException(ResultCode.PERMISSION_NO_ACCESS);
        }
    }
}
